...
class T2ProvisionerK8S(ProvisionerK8S):
   ...
   def _get_priority_class(self, attrs):
      ...
      if pc==None:
         if int(attrs['GPUs'])>0:
            # GPU opportunistic at higher level, since CMS contributed some GPUs
            pc = 'nice'
         else:
            # CPU opportunistic should always be lowest priority
            pc = 'opportunistic'
      return pc

   def _augment_labels(self, labels, attrs):
      """Add any additional labels to the dictionary (attrs is read-only)"""
      labels['ucsdt2-provisioner'] = 'wn'
      return
   ...
   def _augment_node_selectors(self, node_selectors, attrs):
      ...
      # limit to us-west for the time being, due to CMS T2 users using data services 
      node_selectors['topology.kubernetes.io/region'] = 'us-west'
      return
